...
===============================================================================
Uses Gemini 2.5 Flash (or any Gemini model) via Vertex AI.
...
def init_vertex() -> None:
    """Initialise Vertex AI SDK with project and EU location."""
    aiplatform.init(
...
def chat_stream(prompt: str, model_id: str) -> dict:
    ...
    config = GenerationConfig(max_output_tokens=512, temperature=0.7)
    ...
    for chunk in model.generate_content(prompt, generation_config=config, stream=True):
        ...
        # usage_metadata is populated on the last chunk
